        ...
        if grnd_index == "distinct":
            ...
            if not good_match:
                ...
                if is_text_match(grnd_index.key_str, schema.primary_keys[tbl_name]):
                    grounding[grnd_index] = [
                        GroundingKey.make_column_grounding(tbl_name, schema.primary_keys[tbl_name])]
                    good_match = True
        elif grnd.issortdir():
...
def add_grounding_with_check(grounding_index, grounding_key, grounding_out):
    if grounding_key is not None:
        if grounding_index in grounding_out:
            assert grounding_key in grounding_out[
                grounding_index], f"Trying to ground {grounding_index} to {grounding_key}, but have {grounding_out[grounding_index]}"
        else:
...
def ground_filter(head_grounding, sql_unit, i_op, qdmr, sql_spider_data, table_data, grounding_out):
    ...
    op_grounder[qdmr.ops[i_op_source]](head_grounding, sql_unit, i_op_source, qdmr, sql_spider_data, table_data,
                                       grounding_out)
...
def ground_union(head_grounding, sql_unit, i_op, qdmr, sql_spider_data, table_data, grounding_out):
    ...
    if len(head_grounding) == len(args):
        for grnd, arg in zip(head_grounding, args):
            ...
            op_grounder[qdmr.ops[i_op_arg]]([grnd], sql_unit, i_op_arg, qdmr, sql_spider_data, table_data,
                                            grounding_out)
    else:
...
def ground_intersection(head_grounding, sql_unit, i_op, qdmr, sql_spider_data, table_data, grounding_out):
    ...
    for arg in [args[0]]:  # only the first arg of intersection has to be grounded to the same place as the output
        i_op_arg = QdmrInstance.ref_to_index(arg, max_index=i_op)
        op_grounder[qdmr.ops[i_op_arg]](head_grounding, sql_unit, i_op_arg, qdmr, sql_spider_data, table_data,
                                        grounding_out)
...
def ground_sort(head_grounding, sql_unit, i_op, qdmr, sql_spider_data, table_data, grounding_out):
    ...
    op_grounder[qdmr.ops[i_op_data_arg]](head_grounding, sql_unit, i_op_data_arg, qdmr, sql_spider_data, table_data,
                                         grounding_out)
    ...
    if sql_unit["orderBy"]:
        ...
        if sort_arg_grnds:
            op_grounder[qdmr.ops[i_op_sort_arg]](sort_arg_grnds, sql_unit, i_op_sort_arg, qdmr, sql_spider_data,
                                                 table_data, grounding_out)
    else:
...
def ground_group(head_grounding, sql_unit, i_op, qdmr, sql_spider_data, table_data, grounding_out):
    ...
    if len(head_grounding) == 1:
        if sql_unit["groupBy"]:
            assert args[0] == head_grounding[0][
                0], f"GROUP: Have aggregator {head_grounding[0][0]} in SQL but {args[0]} in QDMR"
            # removing the aggregator
            grnd = ("none", head_grounding[0][1])

            op_grounder[qdmr.ops[i_op_data]]([grnd], sql_unit, i_op_data, qdmr, sql_spider_data, table_data,
                                             grounding_out)

            grnds = [parse_spider_col_unit(col_unit, sql_unit, table_data) for col_unit in sql_unit["groupBy"]]
            try:
                op_grounder[qdmr.ops[i_op_index]](grnds, sql_unit, i_op_index, qdmr, sql_spider_data, table_data,
                                                  grounding_out)
            except:
                pass
        else:
            op_grounder[qdmr.ops[i_op_data]](head_grounding, sql_unit, i_op_data, qdmr, sql_spider_data, table_data,
                                             grounding_out)
    else:
        if sql_unit["groupBy"]:
            ...
            op_grounder[qdmr.ops[i_op_data]]([head_grounding[1]], sql_unit, i_op_data, qdmr, sql_spider_data,
                                             table_data, grounding_out)
            op_grounder[qdmr.ops[i_op_index]](grnds, sql_unit, i_op_index, qdmr, sql_spider_data, table_data,
                                              grounding_out)
        # else:
    ...
    if args.spider_idx is not None:
        ...
        for qdmr_name, qdmr in dataset_break:
            ...
            try:
                ...
                print(
                    f"ERROR: {error_details['type']}:{error_details['message']}, file: {error_details['file']}, line {error_details['line_number']}")
