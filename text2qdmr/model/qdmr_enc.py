...
def preprocess_schema_uncached(schema,
    ...
    for i, column in enumerate(schema.columns[start_idx:]):
        if pretrained_type == 'bert' or pretrained_type == 'distilbert'  or pretrained_type == 'bart':
...
class BreakFullEncoderBertPreproc(abstract_preproc.AbstractPreproc):
    ...
    def __init__(
        ...
        if self.pretrained_version == 'bert':
            ...
            self.pretrained_modelname = 'xlm-roberta-large'

        elif self.pretrained_version == 'bart':
            self.pretrained_modelname = 'facebook/bart-large'
